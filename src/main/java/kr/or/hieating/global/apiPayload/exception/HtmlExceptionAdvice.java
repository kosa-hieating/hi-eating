...
public class HtmlExceptionAdvice {
  ...
  public String validation(ConstraintViolationException e, HttpServletRequest request, Model model) {
    String errorMessage = e.getConstraintViolations().stream()
        .map(constraintViolation -> constraintViolation.getMessage())
        .findFirst()
        .orElse("검증 에러가 발생했습니다.");

    log.warn("HTML Page Validation Exception occurred at [{}]: {}", request.getRequestURI(), errorMessage);
  ...
  public String handleMethodArgumentNotValid(MethodArgumentNotValidException e, HttpServletRequest request, Model model) {
    ...
    log.warn("HTML Page MethodArgumentNotValid Exception occurred at [{}]: {}", request.getRequestURI(), errors);
  ...
  public String handleHtmlGeneralException(GeneralException e, HttpServletRequest request, Model model) {
    log.warn("HTML Page Business Exception occurred at [{}]: {}", request.getRequestURI(), e.getMessage());
